...
const MessagePage = () => {
  ...
  const handlUploadImage = async(e)=>{
  ...
  const handlUploadVideo = async(e)=>{
    const file = e.target.files[0];
    setLoading(true)
    const uploadPhoto = await uploadFiles(file);
    setLoading(false)
      ...
      <header className='sticky top-0 h-16 bg-white flex justify-between items-center px-4'>
        <div className='flex items-center gap-4 '>
          ...
          <div>
            ...
            <p className='-mt-2 text-sm'>{dataUser?.online ? <span className='text-green'>online</span> : 'offline'}</p>
          </div>
        </div>
        <div>
          <button className='cursor-pointer hover:text-primary'><BsThreeDotsVertical/></button>
        </div>
      </header>
